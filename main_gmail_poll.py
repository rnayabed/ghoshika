...
# This MUST match the SCOPES used in google_auth_gen.py
...
def refresh_gmail_creds(creds):
    ...
    try:
        ...
        #         # This is a bit of a workaround. Ideally, the token.json from google_auth_gen.py
        ...
        print("Please run google_auth_gen.py to re-authenticate and generate a new token.json.")
        return None
...
def get_gmail_service():
    ...
    Assumes token.json is generated by google_auth_gen.py.
    """
    creds = None
    if not os.path.exists(TOKEN_FILE):
        print(f"ERROR: Token file '{TOKEN_FILE}' not found.")
        print(f"Please run google_auth_gen.py first to generate '{TOKEN_FILE}'.")
        return None, None
    ...
    try:
        ...
        print(f"Please ensure '{TOKEN_FILE}' is valid or run google_auth_gen.py to regenerate it.")
        return None, None
    ...
    if not creds.valid:
        if creds.expired and creds.refresh_token:
            ...
            print(f"Please run google_auth_gen.py to generate a new '{TOKEN_FILE}'.")
            return None, None
            ...
            if (current_time - last_creds_save_time) > SAVE_CREDS_INTERVAL_SECONDS:
                if creds and creds.valid and creds.refresh_token: # Only try to refresh if we have a refresh token
                    ...
                    if refreshed_creds:
                        ...
                        # which will then guide user to run google_auth_gen.py if token.json is problematic.
                    ...
                    print(f"If issues arise, run google_auth_gen.py to get a token with a refresh token.")
